...
import { FormShiftSchema } from "@/src/validation/formShift.schema"; // 🌟 Importato per validare prima dell'invio
...
export default function EditShift() {
  ...
  const { loading, imageUrl, setImageUrl, shiftData } = useFetchShift(id, setForm) as any;
  ...
  useEffect(() => {
    if (shiftData) {
      ...
      if (shiftData.required_workers) setWorkerCount(Number(shiftData.required_workers));
    }
  }, [shiftData]);
  ...
  const onUpdateSubmit = () => {
    // Prepariamo l'oggetto completo per la validazione Zod prima di salvare
      ...
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingTop: insets.top + 10, paddingBottom: 120 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          ...
          <Text style={[styles.subtitle, { color: theme.secondaryText }]}>
            {businessType === "staffing" ? "Update personnel request details." : "Update your hospitality shift details."}
          </Text>
        </View>
